...
CSerialPort::CSerialPort()
{
    ...
    m_arSigCmd = new SigCmd[CMD_NUM];
    for (int i = 0; i < CMD_NUM; i++)
    {
        memset(m_arSigCmd[i].buf, 0, CMD_LEN);
...
//							strlen((char*)port->m_szWriteBuffer),	// Length of message to send
        ...
        for (int i = 0; i < CMD_NUM; i++)
        {
            if (port->m_arSigCmd[i].bToBeSend == true)
            {
                if (siglen + port->m_arSigCmd[i].Length > 1024)
                {
                    break;
                }
                memcpy(sigbuf + siglen, port->m_arSigCmd[i].buf, port->m_arSigCmd[i].Length);
                siglen += port->m_arSigCmd[i].Length;
                port->m_arSigCmd[i].bToBeSend = false;
            }
        }
        bResult = WriteFile(port->m_hComm, // Handle to COMM Port
        ...
        /*while(!bEnd)
        {
            ...
            {
                bResult = WriteFile(port->m_hComm,				// Handle to COMM Port
                                    port->m_szWriteBuffer+port->m_SBget,	// Pointer to message buffer in calling
        finction port->m_SBput - port->m_SBget,	// Length of message to send
                                    &BytesSent,					// Where to store the number of bytes sent
                                    &port->m_ov);
                port->m_SBget = port->m_SBput;
                if (!bResult)
                {
                    break;
                }
            ...
            {
                //将缓冲尾部发送完，但是不给bEnd赋值，造成第二次发送
                bResult = WriteFile(port->m_hComm,				// Handle to COMM Port
                                    port->m_szWriteBuffer+port->m_SBget,	// Pointer to message buffer in calling
        finction port->m_nWriteBufferSize - port->m_SBget,	// Length of message to send
                                    &BytesSent,					// Where to store the number of bytes sent
                                    &port->m_ov);
                                    port->m_SBget = 0;
                if (!bResult)
        ...
        */
        ...
        // 		bResult = WriteFile(port->m_hComm,				// Handle to COMM Port
        // 			port->m_szWriteBuffer,	// Pointer to message buffer in calling finction
        // 			port->m_lenToSend,	// Length of message to send
        // 			&BytesSent,					// Where to store the number of bytes sent
        // 			&port->m_ov);
        //
        // 		//显示发送
        // 		if (port->pSendList != NULL)
        // 		{
        // 			CString strData,str;
        // 			strData = "";
        // 			int i;
        // 			for (i=0;i<port->m_lenToSend;i++)
        // 			{
        // 				str.Format("%.2X ",(unsigned char)(port->m_szWriteBuffer[i]));
        // 				strData += str;
        // 			}
        // 			port->pSendList->AddString(strData);
        // 			port->pSendList->SetCurSel(port->pSendList->GetCount()-1);
        // 		}
        /////////////////////////////////////////////////////////////////
...
BOOL CSerialPort::Create(int inCom)
{
    BOOL res = InitPort(NULL, inCom);
    if (res == TRUE)
    {
        StartMonitoring();
    }
...
BOOL CSerialPort::Create(int inCom, int BaudRate)
{
    BOOL res = InitPort(NULL, inCom, BaudRate);
    if (res == TRUE)
    {
        StartMonitoring();
    }
