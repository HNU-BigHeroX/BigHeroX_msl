...
BYTE CVoyCmd::m_CalSum(BYTE *inBuf, int length)
{
    int temp = 0;
    for (int i = 0; i < length; i++)
        temp += inBuf[i];
...
void CVoyCmd::m_GenerateSigCmd(UINT inIndex, UCHAR inID, UCHAR inLen, UCHAR inMode, UCHAR inMethod, UCHAR *data)
{
    ...
    if (m_arSigCmd == NULL)
    {
        pSendList->AddString("m_arSigCmd == NULL 发送缓冲为空");
        pSendList->SetCurSel(pSendList->GetCount() - 1);
        return;
    }

    if (inIndex >= CMD_NUM || inIndex < 0)
    {
        pSendList->AddString("inIndex >= CMD_NUM 指令序列超限");
        pSendList->SetCurSel(pSendList->GetCount() - 1);
        return;
    }

    m_arSigCmd[inIndex].buf[0] = (UCHAR)0x55;
    m_arSigCmd[inIndex].buf[1] = (UCHAR)0xaa;
    m_arSigCmd[inIndex].buf[2] = inID;
    m_arSigCmd[inIndex].buf[3] = inLen;
    m_arSigCmd[inIndex].buf[4] = inMode;
    m_arSigCmd[inIndex].buf[5] = inMethod;
    if (inLen > 0 && data != NULL)
    {
        memcpy(&(m_arSigCmd[inIndex].buf[6]), data, inLen);
    }
    m_arSigCmd[inIndex].buf[inLen + 6] = m_CalSum(m_arSigCmd[inIndex].buf, inLen + 6);

    // 发送长度
    m_arSigCmd[inIndex].Length = inLen + 7;

    // 设置发送标记
    m_arSigCmd[inIndex].bToBeSend = true;
...
UINT CVoyCmd::DaemonThread(LPVOID pParam)
{
    ...
    while (1)
    {
        if (pcmd->m_nDataCount > 0)
        {
            ...
            pcmd->bNoReply = false;
            //	}
            pcmd->m_nRecvCntLog = pcmd->m_nDataCount;
            pcmd->m_nDataCount = 0;
        }
        else
        {
...
void CVoyCmd::GKSave(int st)
{
    static UCHAR val[2];
    m_Split2Bytes(val, (short)st);

    // 55 AA 38 02 09 70 TH TL SUM				// 守门网弹起
    // m_GenerateSendBuffer(0x38,0x70,2,0x09,(UCHAR*)val);
    m_GenerateSigCmd(SIG_KICK_INDEX, 0x38, 0x00, 0x0A, 0x70, (UCHAR *)val);
}
