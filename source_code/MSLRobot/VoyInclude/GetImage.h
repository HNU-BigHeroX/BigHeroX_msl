...
class CGetImage : public IBehavior
{
  ...
  public:                  // 2012
    ...
    double jdx;
    double jdy;
    double jieqiuRx; // 记录接球 球进入视野点时的 机身位置
    double jieqiuRy;
    double jieqiuRa;
    double jdRx;        // 跟踪点x 速度跟踪用接球 输出跟踪位置
    double jdRy;        // 跟踪点y
    double jdRr;        // 跟踪点到jd距离有可能负号
    double jdRa;        // 未用
    double jdRcr;       // 跟踪点到当前机体距离
    double jdRcrDjdRrP; //
    double jieqiuBSx;   // 记录接球 离开进入视野点 开始动作接球时的 球位置
    double jieqiuBSy;
    double jieqiuRSx; // 记录接球 离开进入视野点 开始动作接球时的 机身位置
    double jieqiuRSy;
    double jieqiuRSa;
    double jieqiuW; // 记录旋转修正量
  ...
  protected:
    ...
    int bianjiejl(double jiao, double jizhun);
  ...
  public:
    ...
    CDebug_Display Debug_D[52];     // 调试信息图形显示用类[0~41 2014] [42~52 2015]
