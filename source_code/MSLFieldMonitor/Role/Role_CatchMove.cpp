#include "stdafx.h"
#include "Role_CatchMove.h"

CRole_CatchMove::CRole_CatchMove()
{
}
...
void CRole_CatchMove::Reset()
{
}

void CRole_CatchMove::Play(stDataSummary *inDataSummary, uRole *inRole)
{
    ...
    int nBestID = 0;
    int nTmpDist = 0;
    // 找已经存在的CTRL_PASS_MOVE队员，维持角色稳定性
    ...
    if (nBestID > 0 && nBestID <= AG_NUM && pCoachUDP != NULL)
    {
        ...
        pCoachUDP->m_ToAgent[nBestID - 1].CtrlCmd(CTRL_CATCH_MOVE, inDataSummary->cGlobalBall.x,
