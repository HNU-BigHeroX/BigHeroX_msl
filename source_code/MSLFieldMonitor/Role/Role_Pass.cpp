...
void CRole_Pass::Reset()
{
}

void CRole_Pass::Play(stDataSummary *inDataSummary, uRole *inRole)
{
    ...
    nBestID = m_GetIDInStatus(CTRL_PASS, inDataSummary->agent, false);
    if (nBestID < 0)
    {
    ...
    {
        pCoachUDP->m_ToAgent[nBestID - 1].PassCmd(inRole->nDest_x, inRole->nDest_y);
        // 标记该队员已经分配行为
