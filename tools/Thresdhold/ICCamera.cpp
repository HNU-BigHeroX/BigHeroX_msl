...
static void cbFrameReady(HGRABBER hGrabber, unsigned char *pData, unsigned long frameNumber, void *Data)
{
    ...
    ///////////////////////////////////////////////
    // 画面移动
    // 	memset(pCam->pTmpBuf,0,640*480*3);
    // 	int nWidthLen = (640-pCam->m_nOffSetX)*3;
    // 	int nStartX = pCam->m_nOffSetX*3;
    // 	int nHeightLen = (480-pCam->m_nOffSetY);
    // 	int nStartY = pCam->m_nOffSetY;
    // 	for (int i=0;i<nHeightLen;i++)
    // 	{
    // 		memcpy(
    // 			pCam->pTmpBuf + (i*640*3) /*+ nStartX*/,
    // 			pData + (i*640*3),
    // 			nWidthLen
    // 			);
    // 	}
...
void CICCamera::SetCmd(CVoyCmd *inpCmd)
{
